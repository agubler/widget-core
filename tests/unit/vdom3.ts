...
describe('vdom', () => {
	...
	describe('widgets', () => {
		...
		it('A', () => {
			...
			console.log('');
			console.log(root.childNodes[0].childNodes[0].data);
			console.log(root.childNodes[1].childNodes[0].data);
			console.log(root.childNodes[2].childNodes[0].data);
			console.log(root.childNodes[3].childNodes[0].data);
		});
	...
	describe('properties', () => {
		...
		describe('classes', () => {
			...
			// 	const [ Widget, meta ] = getWidget(v('div', { classes: null }));
			// 	const div = document.createElement('div');
			// 	dom.merge(div, widget, { sync: true });
			// 	assert.strictEqual(div.className, '');
			// });
	...
	describe('children', () => {
		...
		it('allows a contentEditable tag to be altered', () => {
			...
			(div.childNodes[0] as Element).removeChild((div.childNodes[0] as Element).childNodes[0]);
			handleInput({ currentTarget: div.childNodes[0] as Element });
			meta.setRenderResult(renderDNodes());

			(div.childNodes[0] as Element).innerHTML = 'changed <i>value</i>';
			handleInput({ currentTarget: div.childNodes[0] as Element });
			meta.setRenderResult(renderDNodes());

			assert.strictEqual((div.childNodes[0] as Element).innerHTML, 'changed <i>value</i>');
		});

...
function getWidget(renderResult: RenderResult | Render) {
	return new class extends WidgetBase {
		...
		public get onElementCreatedStub() {
			return this._onElementCreatedStub;
		}

		public get onElementUpdatedStub() {
			return this._onElementUpdatedStub;
		}
...
describe('vdom', () => {
	...
	describe('node callbacks', () => {
		...
		it('on element created and updated callbacks are called for nodes with keys', () => {
			...
			const projection = dom.create(widget);
			resolvers.resolve();
			assert.isTrue(widget.onElementCreatedStub.called);
			assert.isTrue(widget.onElementCreatedStub.calledWith(projection.domNode.childNodes[0] as Element, 0));
			widget.renderResult = v('div', { key: 0 });
			projection.update();
			resolvers.resolve();
			assert.isTrue(widget.onElementUpdatedStub.called);
			assert.isTrue(widget.onElementUpdatedStub.calledWith(projection.domNode.childNodes[0] as Element, 0));
		});
